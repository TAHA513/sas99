...
export default function StaffDashboard() {
  ...
  const { data: todaySales, isLoading: salesLoading } = useQuery({
    ...
    enabled: user?.role === "sales_staff" || user?.role === "manager",
  });
  ...
  const { data: lowStockProducts, isLoading: productsLoading } = useQuery({
    ...
    enabled: user?.role === "inventory_staff" || user?.role === "manager",
  });
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-4">
          {(user?.role === "inventory_staff" || user?.role === "manager") && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {/* Show sales stats only for sales staff and managers */}
        {(user?.role === "sales_staff" || user?.role === "manager") && (
          <Card className="p-6">
        ...
        {/* Show appointments only for sales staff and managers */}
        {(user?.role === "sales_staff" || user?.role === "manager") && (
          <Card className="p-6">
            <div className="flex items-center gap-4">
              <div className="p-4 bg-primary/10 rounded-full">
        ...
        {/* Show inventory alerts only for inventory staff and managers */}
        {(user?.role === "inventory_staff" || user?.role === "manager") && (
          <Card className="p-6">
            <div className="flex items-center gap-4">
      ...
      {/* Show sales table only for sales staff and managers */}
      {(user?.role === "sales_staff" || user?.role === "manager") && (
        <Card className="p-6">
      ...
      {/* Show appointments table only for sales staff and managers */}
      {(user?.role === "sales_staff" || user?.role === "manager") && (
        <Card className="p-6">
          <div className="flex justify-between items-center mb-4">
      ...
      {/* Show low stock products table only for inventory staff and managers */}
      {(user?.role === "inventory_staff" || user?.role === "manager") && (
        <Card className="p-6">
          <div className="flex justify-between items-center mb-4">
